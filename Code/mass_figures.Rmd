...
marine_mass_p <- aggf1RF %>%
  ...
  group_by(response) %>% mutate(x = sum(pred),
                                xfirst = suppressWarnings( min(num_pred[pred==1]) )
                                ) %>% ungroup() %>%
  arrange(desc(x)) %>% mutate(response = fct_inorder(response))
...
fig_marine_mass_p_xfirst <- marine_mass_p %>%
  ...
  theme(text = element_text(size = 14, color = "black"),
        axis.title.y = element_blank(), axis.text.y = element_blank())
fig_marine_mass_p_xfirst +
  (ent %>%
     mutate(medium = factor(medium, levels = levels((marine_mass_p%>% mutate(response =fct_reorder(response, xfirst)))$response))) %>%
...
### RF performance
```{r}
marine_baseline_stats <- read_performance_scores("../Results/MIP_marine_baseline_20230905_stats-acc.csv") %>%
  left_join(., read_performance_scores("../Results/MIP_marine_baseline_20230905_stats-bacc.csv"), by = c("num_predictors", "i", "predictors", "mass_prediction")) %>%
  left_join(., read_performance_scores("../Results/MIP_marine_baseline_20230905_stats-f1weighted.csv"), by = c("num_predictors", "i", "predictors", "mass_prediction")) %>%
  left_join(., read_performance_scores("../Results/MIP_marine_baseline_20230905_stats-jaccard.csv"), by = c("num_predictors", "i", "predictors", "mass_prediction")) %>%
  left_join(., read_performance_scores("../Results/MIP_marine_baseline_20230905_stats-cohen_kappa.csv"), by = c("num_predictors", "i", "predictors", "mass_prediction")) %>%
  left_join(., read_performance_scores("../Results/MIP_marine_baseline_20230905_stats-mcc.csv"), by = c("num_predictors", "i", "predictors", "mass_prediction")) %>%
...
# read in f1 scores
bacdive1f1 <- read_csv("MIP_classify_RF_f1_score_agg-bacdive1.csv") %>%
  dplyr::rename(num_predictors = `# of predictors`) %>%
  mutate(response = str_match(response, "MU:(.+)_builds acid from")[,2]) %>%
